/**
 * Base tests — verify existing board data behaviour that must not regress.
 */
...
describe("normalizeBoardData — existing behaviour", () => {
	it("returns null for non-object input", () => {
		...
		expect(normalizeBoardData("str")).toBeNull();
	});
	...
	it("normalizes a valid card without crashing", () => {
		const raw = {
			columns: [
				{
					id: "backlog",
					cards: [{ id: "t1", title: "Task", prompt: "Task", baseRef: "main" }],
				},
			],
		};
		const board = normalizeBoardData(raw);
